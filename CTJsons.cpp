...
void ParserAndDump_TJson(std::string &canada, std::string &citm_catalog, atomic<int> &iTotal, atomic<int> &iFalseCount, mutex &mt, std::atomic<long long> &elapsed) {
	...
	for (int i = 0; i < 10; i++) {
		...
		oTJSON.RootParse(strCitmCanadaSrc.c_str());
		oTJSON.Stringify(cBuffer, iSize);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	oTJSON.RootParse("{}");
	oTJSON.BackToRoot();
	oTJSON.AddItem("key1", "value1");
	oTJSON.AddItem("key2", "value2");
	oTJSON.AddItem("key3", "value3");
	oTJSON.AddArray("Array 1");
	oTJSON.GetNodeByKey("Array 1");
	oTJSON.AddArrItem("Item1");
	oTJSON.AddArrItem("Item2");
	oTJSON.AddObject("SubDict");
	oTJSON.AddArray("SubArray");
	oTJSON.Stringify(cbuff, i_c); cout << "Stringify: " << cbuff;  // "Array 1": ["Item1", "Item2", "SubArray": [], "SubDict": {}]
	oTJSON.BackToRoot(); 
	oTJSON.Stringify(cbuff, i_c); cout << "\nStringify: " << cbuff; // {"Array 1": ["Item1", "Item2", "SubArray":[], "SubDict" : {}], "key1" : "value1", "key2" : "value2", "key3" : "value3"}
	oTJSON.RemoveItem("Array 1");
	oTJSON.Stringify(cbuff, i_c); cout << "\nStringify: " << cbuff; // {"key1": "value1", "key2": "value2", "key3": "value3"}
	//*/

	//Test_TJS_Node("Debug.json", true);

	//For Python # elapsed_time:  0.121300005913
	
	TestThreadSafe_TJSon(); // In debug build:	 TestThreadSafe_CJSon, iTotal: 37060 iFalseCount: 0 dAvrElapsed: 432 milliseconds
							// In release build: TestThreadSafe_CJSon, iTotal: 3680 iFalseCount: 0 dAvrElapsed: 110 milliseconds
							// TestJson.py ==> elapsed_time:  0.221400001049 TrueCnt : 100 FailedCnt : 0
							// Python json is written in c and it seem be fastest with other libs exlude mine.
