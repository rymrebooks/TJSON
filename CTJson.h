/*
Target design:
	Fastest and steadity
	Long time excutation whithout memory fragmentation.
	Threadsafe.
	...
	Suggestion using: each thread should have a TJSON object to use as global util
	Using: just include header file, TJson.h
Author: Trinhdc
Donate: rymrebooks@gmail.com @ paypal !
License: BSD
Version: 1.0.0.0
RoadMap:
	More test to get bugs.
	Listen to users for improving
Bugs:
...
bool SearchNode(const char *cKey, TNodeData **arr, int iItemCnt, int &iLocation) {
	int low = 0, high = iItemCnt - 1, midpoint = 0;
	while (low <= high) {
		midpoint = low + (high - low) / 2;
		if (IsEqualStr(arr[midpoint]->Key, cKey)) {
			iLocation = midpoint;
			return true; //break; //cout << search << " found at location " << midpoint + 1 << "\n";
		}
		else if (FirstIsLessThanLast(cKey, arr[midpoint]->Key)) {
			high = midpoint - 1;
		}
		else {
			low = midpoint + 1;
...
struct TJSNode : public TNodeData {
	...
	inline bool IsExistKey(const char *cKey, int &iLocation) {
		...
		//*/
		return SearchNode(cKey, (TNodeData **)Nodes, ItemIDCnt, iLocation);
	}
	...
	void QSortKeys() {//TJSNode **Nodes
		if ((Tag == JVL_OBJECT || Tag == JVL_ARRAY)&& ItemIDCnt > 0) {
...
struct TJSON {
	...
	TJSON *operator[](const char *cKey) { // Implement later
		...
		while (1) {
			...
			if (strInput == "[p]") {
				...
				if (oCrrNode->IsExistKey(strInput.c_str())) {
					cout << "\n            The key is existing. key: " << strInput.c_str();
				}
				else {
					cout << "\n            IsExistKey => The key dont existing.";
				}
			}
			else if (strInput == "[v]") {
				cout << "\n            Enter key: "; cin >> strInput;
